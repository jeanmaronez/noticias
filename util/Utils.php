...
final class Utils {
    ...
    public static function createLink($page, array $params = array()) {
        ...
        // NOTICIA add support for Apache's module rewrite
    ...
    /**
     * Get {@link Noticia} by the identifier 'id' found in the URL.
     * @return Noticia {@link Noticia} instance
     * @throws NotFoundException if the param or {@link Noticia} instance is not found
     */
    public static function getNoticiaByGetId() {
        $id = null;
        try {
            $id = self::getUrlParam('id');
        } catch (Exception $ex) {
            throw new NotFoundException('No NOTICIA identifier provided.');
        }
        if (!is_numeric($id)) {
            throw new NotFoundException('Invalid NOTICIA identifier provided.');
        }
        $dao = new NoticiaDao();
        $noticia = $dao->findById($id);
        if ($noticia === null) {
            throw new NotFoundException('Unknown NOTICIA identifier provided.');
        }
        return $noticia;
    }
